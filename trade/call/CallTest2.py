...
while True:
    ...
    for ticker in tickers:
        ...
        if ma3.iloc[-2] < ma15.iloc[-2] and ma3.iloc[-1] > ma15.iloc[-1]: #and iCount == 0:
            upbit.buy_market_order(ticker, 50000)
            iCount = 1
#         if ma15.iloc[-1] > ma5.iloc[-1] and iCount == 1 and Rsi.diff()[-1] < -5:
#             time.sleep(1)
...
#         print("<<<<<<<<  " + ticker + " >>>>>>>>>>>>>>>>")
#         print(str(ma15[-1]) + "  > : " + str(ma15[-2])  + "  ??? " + str(ma5Diff.iloc[-1]))
#         # if ma15Diff[-1] >  0:     #   15평이 상승 추세일때
#         if ma15.iloc[-1] > ma15.iloc[-2]:     #   15평이 상승 추세일때
#             print(ticker + " -> " + str(df['close'][-1]))
#             print("Rsi.diff()[-1] : " + str(Rsi.diff()[-1]))
#             if Rsi.diff()[-1] > 0:
#                 print("nrsi : " + str(nrsi))
#                 if nrsi < 50:
#                     print("evg : " + str(evg * 2 / 3) + " || df[volume][-1] : " + str(df["volume"][-1]))
#                     if (evg * 2 / 3) < df["volume"][-1]:                      #   거래량이 평균 이상일때                
#                         upbit.buy_market_order(ticker, 6000)
