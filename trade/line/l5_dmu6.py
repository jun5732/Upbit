...
while True:
    ...
    # if len(balances) > 1:
    #     continue
    # vas = pd.DataFrame(columns=['ticker','open','high','low','close','volume'])
    vas = []
    for ticker in tickers:        
        df = pyupbit.get_ohlcv(ticker, count=1)
        # va = [{'ticker' : ticker, 'volume' : int(df['volume'][0])}]
        va = ({'ticker':ticker,'volume':int(df['volume'][0])})
        # va = ({'ticker':ticker,'open':df['open'][0],'high':df['high'][0],'low':df['low'][0],'close':df['close'][0],'volume':int(df['volume'][0])})
        vas.append(va)
        time.sleep(0.1)
